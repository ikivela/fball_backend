...
async function getTeamGames(_season) {
  ...
  if (fs.existsSync(basepath + 'nibacos_games.json')) {
    ...
    const group_urls = levels.map((_level) => {
      return `${fb_groups_url}${_level.id}&area=&season=${_season}`;
    });
    const groups_requests = group_urls.map((url) => {
      return axios.post(url);
    });
    let games = [];

    axios.all(groups_requests).then((responses) => {
      groups = responses.map((x) => x.data);
      let statgroups = [];
      groups = groups.forEach((_group) =>
        _group.statgroups.forEach((x) => statgroups.push(x))
      );
      game_urls = statgroups.map((_statgroup) => {
        return `${fb_games_url}${
          _statgroup.StatGroupID
        }&select=&id=&teamid=&rinkid=&rdm=${Math.random()}&season=${_season}`;
      });
      game_requests = game_urls.map((url) => {
        return axios.post(url);
      });
      axios
        .all(game_requests)
        .then((_games) => {
          games = _games.forEach((_game) =>
            _game.games.forEach((x) => games.push(x))
          );
    ...
    //Promise.all(groups).then((_groups) => console.log(_groups));
  }
  /*
