...
async function updateGames(_season, _team) {

	let games = [];
 
	if ( active_groups.length == 0 ) {
		console.warn("Active_groups = 0");
	...
	for (let group of active_groups) {
		...
		if (!games) return console.warn("Games NOT found");

			games = games.filter((x) => {
				return (
					x.HomeTeamName.includes(_team) ||
					x.AwayTeamName.includes(_team)
				);
			});

			if (games.length > 0) {
				console.log(`${group.level.name} [${games.length}]`);
				for (let game of games) {
					currentTeam_games.push({
						...game,
						groupID: group.groupID,
						group: group.name,
						class: group.level.name,
					});
				}
			}
	}

	if ( currentTeam_games.length > 0 ) {
		console.log(
			'Writing %s games for team %s',
			currentTeam_games.length,
			_team
		);
		var filename = `${basepath}${_season}-${_team}_games.json`;
...
async function getTeamGames(params) {
	...
	const updateResults = params.updateGames ? true : false;
	...
	if ( updateResults ) 
		return updateGames(_season, currentTeam);
	...
	if (
			...
			for (let group of groups) {
				...
				if (games) {
					...
					if (games.length > 0) {
						...
						a_groups.push( { groupID: group.StatGroupID, name: group.Name, level: level, season: _season });
						for (let game of games) {
							currentTeam_games.push({
								...game,
								groupID: group.StatGroupID,
		...
		fs.writeFileSync(`${basepath}/config/active_groups.json`, JSON.stringify(a_groups), 'utf8');
		fs.writeFileSync(
...
const argv = yargs
	...
	.option('updateGames', {
		alias: 'g',
		description: 'Update game results, not classes and groups',
		type: 'boolean',
	})
	.help()
	.alias('help', 'h').argv;
